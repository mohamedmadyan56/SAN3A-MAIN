...
const statusColor = (s: string) => {
  const map: Record<string, string> = { COMPLETED: 'bg-green-100 text-green-700', CANCELLED: 'bg-gray-100 text-gray-500' };
...
export default function CraftsmanHistoryPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/users/dashboard/craftsman`, {
      ...
      if (res.data.status === 'success') {
        setJobs(res.data.data.recentJobs || []);
      }
      ...
      <main className="flex-1 p-6 overflow-auto">
        <div className="max-w-4xl mx-auto">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 font-light mt-1">جميع مهامك السابقة</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
              ...
              <h2 className="text-xl font-black text-gray-900 mb-2">لا توجد مهام سابقة</h2>
              <p className="text-sm text-gray-500 font-light">سيظهر هنا تاريخ مهامك</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="divide-y divide-gray-50">
                {jobs.map((job) => (
                  <div key={job._id} className="flex items-center justify-between p-4 px-6 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`text-xs font-bold px-2.5 py-1.5 rounded-full ${statusColor(job.status)}`}>{statusLabel(job.status)}</span>
                      <span className="font-display text-sm text-gray-900">${job.pricing?.totalAmount || 0}</span>
                    </div>
                  </div>
                ))}
              </div>
