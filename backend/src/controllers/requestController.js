...
exports.findNearbyCraftsmen = catchAsync(async (req, res, next) => {
  const request = await prisma.request.findUnique({
    where: { id: req.params.requestId },
    include: { matchingPoolEntries: true },
  });
  if (!request) return next(new AppError('الطلب غير موجود', 404));
...
exports.getMatchResults = catchAsync(async (req, res, next) => {
  const currentRequest = await prisma.request.findUnique({ where: { id: req.params.requestId } });
  if (!currentRequest) return next(new AppError('الطلب غير موجود', 404));
...
exports.acceptRequest = catchAsync(async (req, res, next) => {
  ...
  if (!['craftsman', 'customer'].includes(req.user.role))
    return next(new AppError('غير مسموح', 403));
  if (!selectedCraftsmanId)
    return next(new AppError('يرجى تحديد الفني', 400));
  ...
  if (req.user.role === 'craftsman') {
    ...
    if (!existing) return next(new AppError('الطلب لم يعد متاحاً', 409));
  ...
  if (!currentRequest) return next(new AppError('الطلب لم يعد متاحاً', 409));
...
exports.confirmBooking = catchAsync(async (req, res, next) => {
  ...
  if (!currentRequest) return next(new AppError('طلب غير موجود', 404));
});
